...
positionsToDelete <- which(PR$id %in% DS$userid)
length(positionsToDelete)
length(PR$id)
#positionsToDeletepositionsToDelete <- which(is.element(PR$id,DS$userid)) 
positionsToDelete
PR2 <- PR[-positionsToDelete, ]
...
#que grupos las versiones ?? que tengo que mostrar????
Versiones<-unique(DS$version)
