...
```{r Merge datasets}
  ...
  write.csv(merge.all,"D:/uni/QUART/merged_dataset.csv", row.names = FALSE)
  write.csv(merge.purch,"D:/uni/QUART/merged_justpurch_dataset.csv", row.names = FALSE)
```
...
To see if there are non-playing users, we look for those who had both retention at day 1 and at day 7 as FALSE.
```{r Sum non-playing users}
  non.playing.users <- sum(DS$retention_1 == FALSE & DS$retention_7 == FALSE) ; non.playing.users
```
Finally, to compute the percentage, we just have to multiply the non-playing users for 100 and then divide it for the total users.
...
```{r Visualisation part 2}
 ...
 pie(totalUsersWithMaxRetention,labels=c("Group A (gate_30)","Group B (gate_40)"), main = "Maximum retention comparison between the two groups")
...
```{r Visualisation part 3}
 ...
 aTmp<-paste("Group A (gate_30)",GroupASumGameRounds)
...
degreesOfFredom
```
After calculating the previous values, we realize a comparasion between the observed value and the normal distribution, using the upper tail.
...
h0CanBeRejected
```
For verification of the analysis we do a double check, now using the pvalue and alpha to see if the results are the same
...
Since Tobs isn't in the acceptance area, we can't reject H0. This indicates that we don't have enough evidence to say that adding the gate_40 the engagement is improved.
...
90185 users download the game.

2. Are the two groups of users well balanced in terms of number of players that got version A and version
B of the game?
From the 90.185 users, 44698 played the gate number 30 and 45.487 played the gate number 40.
Which this data we can conclude the groups are balanced in term of number of players.

3. What is the distribution of game rounds in the players’ population? And among the users of version A
and version B of the game separately?

4. What is the value of retention at day 1? (percentage of users that are still active the day after
installation).
The percentage of retention at day 1 is: 44.521%

5. What is the value of retention at day 7? (percentage of users that are still active after one week of
installation).
The percentage of retention at day 7 is: 14.617%

6. Are there users that never played the game? How many? In what percentage?
Yes. There are 46436 that never played the game. This is the 51.49% againsts all the users that downloaded the game.
...
No. We don't have enough evidence to conclude any improvement moving gate lo level 40
...
Finally, on behalf of the authors of this project, Marcel Feliu, Paula Ros and Eduard Corral, we commit ourselves that we have not made any kind of cheat, creating every part of this final project with our own knowledge and abilites.
