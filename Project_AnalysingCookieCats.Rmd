...
```{r Percentage of paying users}
  ...
  percentage.paying.users <- num.users.PR * 100 / PR.AND.DS.USERS
...
```{r Mean and maximum of money spet}
  ...
  # <- max(PR$purch) ; max esta mal, s'ha de mirar el maxim total de compres per usuari, no la compra mes cara
...
```{r Predicting a case of 50 game rounds}
  fifty.gamerounds <- 50
  new.ds <- data.frame(sum_gamerounds=fifty.gamerounds)
  prediction = predict(lm.all, new.ds)
  prediction
  
  prediction1 = predict(lm.just.purch, new.ds)
  prediction1
```
...
```{r Simulation player spending for different game rounds}
  simulation.game.rounds <- seq(from = 0, to = 1000, by = 50)
  new.ds <- data.frame(sum_gamerounds=simulation.game.rounds)
  predicted.simulation <- predict(lm.all, new.ds)
  predicted.simulation
  
  predicted.simulation.1 <- predict(lm.just.purch, new.ds)
  predicted.simulation.1
```
