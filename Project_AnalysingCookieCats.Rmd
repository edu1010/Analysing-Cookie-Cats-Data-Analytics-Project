...
```{r Percentage Purchase Quantities}
  ...
  num.users.PR <- nrow(PR.different.purch) ; num.users.PR
  
  purch.2.29 <- sum(length(PR.different.purch$purch[PR.different.purch$purch == "2.29"])) * 100 / num.users.PR 
  purch.10.99 <- sum(length(PR.different.purch$purch[PR.different.purch$purch == "10.99"])) * 100 / num.users.PR 
  purch.21.99 <- sum(length(PR.different.purch$purch[PR.different.purch$purch == "21.99"])) * 100 / num.users.PR 
  purch.54.99 <- sum(length(PR.different.purch$purch[PR.different.purch$purch == "54.99"])) * 100 / num.users.PR
  purch.109.99 <- sum(length(PR.different.purch$purch[PR.different.purch$purch == "109.99"])) * 100 / num.users.PR
  
  purch.quantities <- c(purch.2.29, purch.10.99, purch.21.99, purch.54.99, purch.109.99)
  
  myLables <- c("2,29$ ", "10,99$ ", "21,99$ ", "54,99$ ", "109,99$ ")
  myLables2 <- paste(round(purch.quantities, 1),"%", sep = "")
  myLables2
...
```{r Percentage of people who has purchases more than 1 time}
  ...
  percentage.multiple.purchases <- repeated * 100 / PR.different.purch
  pct.paying.users <- (PR.different.purch-repeated) * 100 / PR.different.purch ; pct.paying.users
...
```
JFLKDSJLFAÑ
```{r Mean and maximum of money spent}
...
```{r Conversion rate}
  ...
  num.users.PR <- length(table(PR.different.purch))
  
  PR.AND.DS.USERS <- sum(num.users.DS, num.users.PR)
  
  conversion.rate <- (num.users.PR / PR.AND.DS.USERS) * 100
  paste(as.character(round(conversion.rate, 2)), "%")

```
### 4.2. ARPU
```{r ARPU}
  total.revenue <- sum(PR$purch)
  ARPU <- total.revenue / PR.AND.DS.USERS ; ARPU

```
### 4.3. ARPPU
```{r ARPPU}
  ARPPU <- total.revenue / num.users.PR ; ARPPU
...
alpha = qt ( 0.05, df=degreesOfFredom,lower.tail=FALSE )#sumGameRounds40-sumGameRounds30>0 creo
alpha
```
The necesary condition to reject H0 is sumGameRounds gate 40 > sumGameRounds gate 30
```{r tObs  and aplha}
h0CanBeRejected<-Tobs>alpha
h0CanBeRejected
```
...
alpha
```
To reject H0 p needs to be lower than alpha, in this case

Alpha es lo que he puesto que es alpha estoy haciendo algo mal? no entiendo

### 5.4 Conclusion of A/B testing
Since Tobs is in the aceptance area we can reject H0
...
After looking at the results of the graph, we can intuit that there is no direct relationship between playing more rounds and spending more money in the game since, as the graph shows, the biggest purchases are made by those users who play between 0 and 700 rounds. All the users that have played more than approximately 1000 rounds have spent no money in the game.
...
#supuestamente purch es mejor
```

Visualise the model. Explain how the model relates the total amount of purchases with the game rounds.
...
```{r Predicting a case of 50 game rounds}
  ...
  prediction = predict(lm.all, new.ds)
  prediction
```
...
```{r Simulation player spending for different game rounds}
  ...
  simulation <- predict(lm.all, new.ds)
  simulation
